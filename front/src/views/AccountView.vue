<template>
  <div class="grid grid-cols-6 sm:flex sm:flex-col md:flex md:flex-col">
    ...
    <div class="mt-10 col-end-7 col-span-4">
      <h1 class="text-lg underline md:text-center sm:text-center">Vos essais clinique enregistrés</h1>
      <p v-if="status" class="text-red-600">{{ status }}</p>
      <p v-if="error" class="text-red-600">{{ error }}</p>
      <div class="grid grid-cols-2 gap-4 mr-5 mt-10 md:flex md:flex-col md:justify-items-center md:mr-3 md:ml-3 sm:flex sm:flex-col sm:justify-items-center sm:mr-3 sm:ml-3">
        <ul
          v-for="infos in info"
          v-bind:key="infos.id"
          class="mb-7 border"
